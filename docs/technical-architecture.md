...
- 不做大型平台级虚拟化控制面；MyTeam 必须维护 replay / fork 所需的 runtime workspace，并通过 `WorkspaceProvider` 或 host capability 显式声明隔离能力。
...
TeamEngine
  ...
  does not own: concrete business skills, host-server protocols, product-specific workflows, or low-level VM/container control plane
...
WorkspaceProvider
  owns: runtime workspace inspect / snapshot / executable copy / fork / cleanup / isolation capability reporting
  does not own: project personality, agent definitions, skill loading, or hidden security guarantees
...
Tool & Capability Adapters
  ...
  does not own: planning, acceptance, user communication, workspace snapshot/copy lifecycle, or hidden isolation guarantees
```
...
- 工具适配器只在明确的 prepared workspace 与 capability contract 内执行受控动作，不参与任务决策，不隐式提供隔离保证。
- workspace 插件只能通过公开扩展点影响成员、技能和工具，不能导入核心私有模块。

### 3.1 权威契约地图

本文是架构总览，不替代能力 spec。实现和评审时按以下优先级解读契约：

```text
归档后的 openspec/specs/*
  > 当前 active change 的 specs/*/spec.md
  > docs/technical-architecture.md 的架构边界与数据约定
  > README.md / openspec/project.md 的产品定位与红线
  > proposal.md / design.md / tasks.md 的动机、取舍和实施清单
```

当文档冲突时，`spec.md` 中的 SHALL / MUST 场景优先；`proposal.md` 和 `design.md` 只解释为什么和怎么做，不能覆盖 spec；`tasks.md` 只跟踪实施步骤，不能新增行为语义。

| 契约主题 | 权威来源 | 说明 |
|---|---|---|
| 产品定位、核心原则、技术红线 | `README.md`、`openspec/project.md` | 定义 MyTeam 是什么 / 不是什么 |
| public entrypoints、CLI、replay、workspace lifecycle | `openspec/changes/library-first-team-engine/specs/team-engine-entrypoints/spec.md` | `TeamEngine`、默认 CLI、`replay`、`init/check/repair` 的入口语义 |
| session 与 runtime 持久化 | `openspec/changes/align-workspace-storage-genethub/specs/workspace-storage/spec.md` | `.myteam/` 目录、`ChatSession`、`revision`、SessionManager 写入语义 |
| runtime workspace、snapshot、fork、隔离能力声明 | `openspec/changes/workspace-provider/specs/workspace-provider/spec.md` | `WorkspaceProvider`、Git V1、prepared copy、unavailable reason |
| PI-backed InternalAgent 边界 | `openspec/changes/adopt-pi-backed-internal-agent/specs/internal-agent-runtime/spec.md` | PI 类型隔离、prompt 注入、tool/event 映射 |
| 用户可见沟通协议 | `openspec/changes/define-user-visible-communication/specs/user-visible-communication/spec.md` | PM 如何确认、汇报、阻塞、失败和收口 |
| 公共数据字段草案 | 本文第 8 节，后续以 contracts 代码和归档 spec 为准 | 当前阶段用于统一术语，具体实现时应迁入 contracts 层 |
...
不属于 `TeamEngine`：具体调研能力、表格整理能力、网页生成能力、发布托管、用户账号系统、监控运营系统、宿主服务器协议、底层 VM/container 控制面或宿主机权限代理。
...
# workspace lifecycle / diagnostics
myteam init --workspace /path/to/workspace
myteam check --workspace /path/to/workspace --json
myteam repair --workspace /path/to/workspace --json
```
...
- provider 是 MyTeam 维护的 runtime workspace 执行边界；Git V1 worktree 提供文件系统副本隔离，但不自动提供进程、网络或权限隔离。更强隔离必须由 provider 或 host capability 显式声明并进入 replay metadata。
...
TaskRequest
  ...
  -> build ToolRegistry
  -> create TaskContext
  -> WorkspaceProvider.inspect / capture run-start WorkspaceSnapshotRef when supported
  ...
  -> emit TaskResult
  -> persist TaskRecord / Evidence / ReplayCase / WorkspaceSnapshotRef / FailureFingerprint
```

Library API、默认 CLI 和 replay 的区别只在入口来源和结果通道，不在核心执行语义。Workspace `init` / `check` / `repair` 是显式生命周期入口；普通 task run 不得静默初始化或修复损坏 workspace。
...
workspace/
  ...
  .myteam/                 # MyTeam 运行时数据，不作为插件输入
    sessions/              # ChatSession 持久化文件；承载 SessionTranscript 逻辑视图
      {sessionId}.json
      archives/
        {archiveId}.json
    runs/                  # runId 归属的 TaskRecord / EventLog / Evidence / ReplayCase
      {runId}/
        record.json
        events.jsonl
        replay.json
        evidence/
        workspace/         # WorkspaceSnapshotRef / prepared copy metadata
        nodes/             # replayable node state before/after metadata
    memory/                # 预留：跨会话持久化记忆
```
...
- `ChatSession` 是 `.myteam/sessions/` 下的持久化 JSON schema；`SessionTranscript` 是从 `ChatSession.messages`、摘要和 task/run 引用形成的逻辑 transcript；`ConversationSession` 是 public API 返回的轻量投影；`SessionSnapshot` 是 resume 时从持久化 transcript 重建的视图。
- 公共 ID 使用带语义前缀的不透明字符串，由 contracts 层统一生成和校验：`session_`、`turn_`、`task_`、`run_`、`ws_`、`message_`、`evidence_`、`artifact_`、`archive_`。
- `runId` 是执行事实归属的强制字段：`TaskResult`、`TaskEvent`、`Evidence`、`TaskRecord` 和 `ReplayCase` 必须包含 `runId`；`taskId`、`sessionId`、`turnId` 用于关联用户任务和会话，不能替代 `runId`。
...
```json
{
  ...
  "finalMessageRef": "message_xxx"
}
```

`TaskResult.status` 是 public contract 的终态集合。存储层 `Outcome.kind = done` 必须在越过 TeamEngine contract 边界前映射为 `succeeded`；取消必须显式保留为 `cancelled`，不能被折叠成普通 `failed`。
...
- 事件是事实日志，不是 UI 文案。
- 每条 `TaskEvent` 必须包含 `runId`、同一 `runId` 内单调递增的 `seq`、时间、actor 和 type。
- 事件在可用时包含 `taskId`、`sessionId`、`turnId`、`workspaceId`、关联 `subtaskId`、`toolCallId`、`evidenceRef` 或 `errorCode`。
- 错误事件必须含可分类 code、可重试性、影响范围。
- 不在事件中写入 secret、隐藏 system prompt、模型私密 reasoning 或未脱敏 adapter-private ID。
...
- MyTeam 必须维护 `WorkspaceProvider`，用于 inspect、snapshot、prepareCopy、forkCopy、cleanup 和隔离能力报告。
- Git V1 provider 默认提供可 replay / fork 的 worktree 文件系统副本边界；它不是完整进程、网络或权限隔离。
- 更强隔离能力（container、Seatbelt、seccomp、restricted token、远程 volume snapshot 等）可由 host/runtime 或未来 provider 提供，但必须通过 capability 和 replay metadata 显式声明是否可用。
- MyTeam owns capability declaration、permission check、audit event、structured error、workspace boundary metadata 和 replay metadata。
...
- TeamEngine 支持至少两个并发 run，各自拥有独立事件、证据、错误语义和 replay case；同一 session transcript 的并发追加不得丢消息。
