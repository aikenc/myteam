...
MyTeam 的产品定位是"PM + 专业 CLI Agent"的 AI 工作小团队。它需要可靠的单 Agent 推理与工具调用循环，也需要团队级任务生命周期、PM 工作流、CLI Agent 委派、证据、错误与 replay。
...
本变更明确选型：**PI 负责 `InternalAgent` 的核心 `AgentLoop`；MyTeam 负责 Team Orchestration、Task Contract、Workspace/SOUL、Tool/Capability、Evidence/Replay；`CLIAgent` 作为专业执行成员继续保留。**
...
- 保留 `CLIAgent`：Claude Code、Codex、Gemini CLI、browser/data/doc 等专业 CLI Agent 仍是团队成员，不降级为普通工具。
...
- `team-orchestration-runtime`
  - PM / Professional Agent 的团队级工作流边界。
  - PM driver 状态机、委派、验收、收口。
...
- PM 工作流若只靠 prompt 约束，容易漂移；必须有 driver 状态机和结构化 action。
