...
MyTeam TeamWorkflowLoop
  ...
  - PM driver 状态机
  - delegate / plan / verify / finalize / fail
...
Team Orchestration
  - PM Agent
  - PM Driver / workflow state
  - AgentCatalog / member routing
  |
  +--> InternalAgent
...
PM 工作流使用显式 driver / 状态机，不仅靠 prompt 约束：

```text
ProjectState
  -> getTurnDirective()
  -> speaker.runTurn()
  -> extract ProjectAction
  -> applyTurn()
  -> persist state / events / evidence
...
PM 可以是 PI-backed `InternalAgent`，但 PM 的推进语义由 MyTeam driver 验收和持久化。
...
```text
TaskRequest
  -> PM: understand user goal, produce TaskBrief
  -> PM driver: create ProjectState and first directive
  -> PM InternalAgent: plan / delegate / verify / finalize
  -> CLIAgent/InternalAgent members: execute delegated work
  -> PM driver: apply action and state transition
  -> PM: user-friendly final response
...
- Node 版本与依赖升级压力。
- 需要维护 adapter 和事件映射。
- 需要在 MyTeam 层补足权限、证据、replay 与 PM 状态机。
