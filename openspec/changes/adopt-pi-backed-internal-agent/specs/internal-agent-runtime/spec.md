...
- system prompt。
- workspace `SOUL.md`。
- agent-specific `AGENT.md` 或等价 agent prompt。
...
- **GIVEN** workspace 中存在 `SOUL.md` 且 agent profile 指向 `AGENT.md`
- **WHEN** `InternalAgentRuntime` 构造 PI Agent context
- **THEN** PI 的 system/context 输入 SHALL 包含 system prompt、SOUL 内容、AGENT 内容与 capability block
...
`InternalAgentRuntime` SHALL 将 PI AgentLoop 事件映射为 MyTeam `TaskEvent`。映射后的事件 MUST 包含 MyTeam 运行所需字段：`taskId`、`agentId`、`subtaskId`、timestamp、event type、status、相关 tool call id、evidence ref 或 error code。
...
ReplayCase SHALL 至少记录：TaskRequest、workspace 摘要、agent profile 摘要、PI 版本、prompt assets 摘要、TaskEvent、Evidence、工具调用与结果、PM state transitions。
