...
packages/engine/src/
  ...
  workspace-provider/        # runtime workspace inspect / snapshot / copy / fork / cleanup
...
Workspace lifecycle API 属于 public facade / workspace 模块，不属于团队执行 workflow 本身，但必须与默认 CLI 使用同一套 workspace validation 语义：

```ts
function initWorkspace(input: InitWorkspaceInput): Promise<WorkspaceInitResult>;
function checkWorkspace(input: CheckWorkspaceInput): Promise<WorkspaceCheckResult>;
function repairWorkspace(input: RepairWorkspaceInput): Promise<WorkspaceRepairResult>;
```

普通 `TeamEngine.start()` 不得隐式修复损坏 workspace；它只能返回结构化 workspace_error 和修复建议。
...
myteam init --workspace .
myteam check --workspace . --json
myteam repair --workspace . --json
```
...
- 默认 CLI 在普通任务执行前必须执行 workspace `run` 模式校验；缺少关键 runtime 目录或 prompt asset 时返回结构化错误，并提示 `myteam init`、`myteam check` 或 `myteam repair`。
...
replay 不能依赖 CLI 私有日志，必须基于 MyTeam 的 TaskRecord、TaskEvent、Evidence、ReplayCase 和 `WorkspaceSnapshotRef`。MyTeam owns ReplayCase 格式、replay workflow 与 `WorkspaceProvider` 抽象；deterministic / repair replay 必须通过 `WorkspaceProvider.prepareCopy()` 获得可执行 workspace。若 provider 或隔离能力不可用，系统必须给出明确的 unavailable 原因，不能伪装为已复现。
...
`SessionTranscript` 至少包含 `sessionId` 与 `turnId`，并通过公共 MyTeam ID 引用关联 `taskId` / `runId` / event refs / evidence refs / replay refs。`TaskResult`、`TaskEvent`、`Evidence`、`TaskRecord`、`ReplayCase` 必须至少包含 `runId`；需要关联用户任务和会话时同时包含 `taskId`、`sessionId`、`turnId`。PI request/message/tool ID 或 `llmRequestId` 不能作为 public task/run 链接。
...
- 一个 `sessionId` 可以挂载多个并发 `taskId` / `runId`；session transcript 使用 append + revision / cursor 语义维护用户可见连续性，所有 TeamEngine session 写入必须经过 SessionManager / SessionActor 或等价串行化层。
...
MyTeam 不通过全局 session 执行锁来规避并发；run 可并发，但 session transcript 追加必须通过专门的 session mutation serialization 保证不丢消息，workspace 写入、browser profile、artifact、外部 CLI 等共享资源再通过资源级锁和事件/记录隔离保证可解释性。
...
定义 TeamEngine、createTeamEngine、ConversationSession、SessionSnapshot、SessionCursor、StartRunOptions、ActiveRun、RunStatus、EventCursor、RunStatusSnapshot、OutcomeOptions、CancelReason。真实验收路径必须使用可持久化 session store、run store、event log、TaskRecord、Evidence、ReplayCase 和 WorkspaceSnapshotRef；内存 store 仅允许用于单元测试或 prototype，不得作为第一条闭环验收。
