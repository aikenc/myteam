...
- 定义最小接口：`openSession`、`resumeSession`、`forkSession`、`start`、`stream`、`check`、`outcome`、`cancel`，并通过 public facade 提供 workspace `init` / `check` / `repair` 生命周期入口。
...
- 如果 deterministic / repair replay 缺少 `WorkspaceSnapshotRef`、prepared workspace copy 或隔离能力声明，却不显式报 unavailable，会破坏运营自愈的可信度。
