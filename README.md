...
| `TeamEngine` | 会话、并发任务生命周期、事件、工具注册、错误模型、记录与 replay | 不内置具体业务能力，不承载宿主服务器协议，不让入口绕过统一执行语义 |
| `Workspace Layer` | 项目上下文、成员定义、技能、工具声明 | 不保存 secret，不直接修改核心内部状态 |
| `WorkspaceProvider` | 维护 runtime workspace 状态、snapshot、可执行副本、fork、cleanup 和 replay 可用性解释 | 不把 workspace copy 伪装成完整安全隔离；隔离能力必须显式声明并进入 replay 记录 |
...
- replay case 必须记录 `WorkspaceSnapshotRef` 或明确的 workspace-unavailable 原因，避免不可复现时伪装成可自愈。
...
- 不把沙箱、workspace copy 或权限隔离做成隐式黑盒；MyTeam 必须通过 `WorkspaceProvider` 和 capability contract 显式声明运行边界、隔离能力、审计、错误语义和 replay 记录。
- 不用健康检查、mock 或纯 API 成功冒充真实任务完成。
- 不静默吞掉失败、权限缺失、浏览器缺失或工具不可用。

## 文档索引

- [技术架构](./docs/technical-architecture.md)：包含权威契约地图；具体行为以 OpenSpec `spec.md` 的 SHALL / MUST 场景为准。
